...
func formatTime() string {
	...
	var formate = "2006-01-02 15:04:05"
	var now = time.Now()
	var loc, _ = time.LoadLocation("Local")
	var timeFormat = now.In(loc).Format(formate)
	// return fmt.Sprintf(" at %s", timeFormat)
	return timeFormat
}
...
func (l *logger) header(funcName, file string, line int) string {
	...
	return fmt.Sprintf("[%s %s:%d %s]", now.Format("2006-01-02 15:04:05"), shortFile(file), line, funcName)
}
